(** A DSL for creating Html5 documents. *)

(** Html5 attribute type *)
...
(** The core Html5 signature. This can be extended to the full [DSL]
    using the functor [DSL_Of]. *)
module type S = sig
  type t

  module Attr : Attr
  module AttrMap : Map.S with type key = Attr.t
...
(** Efficient string builder for HTML *)
module Printer : S = struct
  type t = Buffer.t
  ...
  let mk_node ~name ?attributes children =
    let csize = List.fold_left (fun tot buf -> tot + Buffer.length buf) 0 children in
    let b0 = Buffer.create csize in
    let (<$) b s = Buffer.add_string b s in
    let (<%) b s = Buffer.add_buffer b s in
    let flip f a b = f b a in
    b0 <$ "<" ^ name;
    begin match attributes with
      | Some attrs when not (AttrMap.is_empty attrs) ->
        flip AttrMap.iter attrs begin fun name value ->
          b0 <$ (" " ^ name ^ "=\"" ^ value ^ "\"")
        end
      | _ -> ()
    end;
    b0 <$ ">";
    List.iter (fun child -> b0 <% child) children;
    b0 <$ "</" ^ name ^ ">";
    b0
              
  let mk_leaf ~name ?attributes () = failwith "noo"
  let mk_attr ~name s = AttrMap.singleton name s

end

(** The [DSL] signature implements a tagless DSL for the
    construction of Html5 node trees. It can be built quickly from
    implementors of the [S] signature. *)
module type DSL = sig
